...
const DonorForm = ({ onDonorAdded, onUpdateDonor, selectedDonor }) => {
    ...
    useEffect(() => {
      if (selectedDonor) {
        ...
        // Clear form fields if no donor is selected
        setName('');
        setBloodGroup('');
        setContact('');
      }
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        if (selectedDonor) {
          // If selectedDonor exists, it means we are in update mode
          ...
          // If selectedDonor is null, it means we are in create mode
    ...
    return (
      <div className="container mx-auto my-8">
        <h1 className="text-3xl font-bold mb-4">{selectedDonor ? 'Update' : 'Donor Registration'}</h1>
